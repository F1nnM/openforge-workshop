 ...
 * driven headlessly in `routes.test.ts`. PR 15 calls them with `useRouter()`.
 */
import type { ManifestOrdinal } from '@/catalog/schema'
 ...
 */
export async function openTileDrawer(router: WorkshopRouter, tile: ManifestOrdinal): Promise<void> {
 ...
 */
export async function showTileInDrawer(router: WorkshopRouter, tile: ManifestOrdinal): Promise<void> {
  await router.navigate({
    to: '/catalog',
    search: (prev) => ({ ...prev, tile }),
    replace: true,
