...
describe('the persisted shape against the shipped templates', () => {
  it('accepts all 40 shipped template ids', () => {
    ...
    // 40, which is the fixtures and nothing else: row E3 appended two assemblies
    // authored in this repo and both are withdrawn.
  ...
  it('bounds a fill map at five entries, which is where “~5x” comes from', () => {
    ...
    // one of the 40 templates has 3 or 5 parts, so a persisted instance carries
    // at most five fills where a version 5 placement carried one identity. It is
    // the bound on the payload rather than the mean of it, which is why row E3's
    // four-slot corridor moved the *set* and not the bound — and why withdrawing
    // it moves the set back and leaves the bound alone again.
