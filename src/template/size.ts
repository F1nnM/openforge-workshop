 ...
 * `wall-on-tile` combinations B2 reports as `fails`; the other 16 inexact slots
 * over-run it, and are the 8 external-corner failures counted once per wall. So
 * the looser predicate buys nothing but candidates the layout then refuses,
 * which is the *needs a choice* the plan's §3.2 wants the auto-fill never to
 * produce.
 *
 ...
 * search hits to 15**. Ten strings for the last 1.26% of the run predicate is
 * not that trade, and {@link RUN_UNREACHABLE} is what it buys. The upgrade is
 * priced and left on the table for **B4/B5**, which own the family table the
 * last 84 records would serve.
 *
...
/**
 * The two tags that make a `size|width` ref mean the run it says.
 *
 * `require: ['size|width|2']` alone is wrong on **214** records and every one of
 * them carries one of these two:
 *
 ...
 *
 * Denying both leaves **zero** false positives across the whole corpus — no
 * record outside these two tags carries a `size|width` that disagrees with its
 * resolved run — which `pipeline/size.test.ts` asserts in that direction as well.
 */
export const RUN_DENY_TAGS: readonly string[] = ['shape|angled|right', 'shape|option|curved_interface']

/**
 * What the zero-byte run predicate cannot reach, enumerated rather than rounded.
 *
 * **84 of the 6,661 records with a run, 1.26%**, and they are three groups:
 *
 ...
 */
export const RUN_UNREACHABLE = 84
