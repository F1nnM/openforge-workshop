# The control position as a property of a placed instance
...
They decide what a click places and then they are forgotten: the position lives
in `usePlanTools`, which is explicit that it is *"a property of what is armed,
not of the room"*.

That is now the wrong boundary. A placed instance was placed *as* an arched door
at 2 x 2, and two surfaces need to know it:

- the **slot editor**, whose candidate lists should offer what the position
  allows rather than all 1,451 walls;
- the **instance itself**, so re-opening a room a week later still says which
  door it is.

## The position is not recoverable from the fills
...
only let them go stale. A position is not in that class.
...
- **A position change re-solves, and may discard a pin.** A pinned fill still
  valid under the new position stays pinned; one that is not is dropped and its
  slot re-solved. Auto fills always re-solve.
- **The position travels in a share link.** A recipient's slot editor is
  filtered the way the author's was.
...
inherit a position its own row cannot express.

### On change: `reSolveInstance`

A new driver beside `reSolveScene`, for one instance and one position.

1. Pose the template — `{...template, tags: [...template.tags, ...position]}`.
2. Per slot, resolve candidates under the posed template with **no sibling
   selections**. That is the widest set the position allows, and using it makes
   the compatibility test a function of the position alone rather than of the
...
interface InstanceReSolve {
  ...
  /** Pins dropped because the new position does not admit them. Reported, never silent. */
...
`slotEditorModel` poses the template before `assemblyState`, which filters every
slot's cards through the mechanism the fold already built. `SlotEditor.tsx`
renders one `AxisControl` per non-empty axis above the slots.
...
A position table and a per-instance index column, through the existing
`writeTable` — the fourth use of a helper that already serves the template, slot
and recipe tables.

```
uvar    position table count     distinct positions in the scene
position table                   count x (uvar byte length, UTF-8 bytes)
position column                  count x uvar (index into the position table)
```

A position is one table entry as a single joined string rather than a list of tag
indices: positions **repeat** across a room — ninety instances of one row share
...
the position, but a surface that contradicts itself.
...
| claim | where |
| --- | --- |
| a placed instance carries the armed position | `src/builder/three/edits.test.ts` |
| version 7 blobs are discarded, not half-read | `src/store/migrations.test.ts` |
| a valid pin survives a position change | `src/template/relock.test.ts` |
| an invalid pin is dropped **and reported** | `src/template/relock.test.ts` |
| auto fills always re-solve | `src/template/relock.test.ts` |
| the editor's lists narrow to the instance's position | `src/builder/panels/slots/slots.test.tsx` |
| the axis controls render above the slots | `src/builder/panels/slots/slots.test.tsx` |
| a position survives a share round trip | `src/share/link.test.ts` |
| the payload prices are re-measured | `src/share/capacity.test.ts` |
| the two size bases agree per slot, per position | `src/template/corpus.test.ts` |
