...
describeFixtures(title, () => {
  ...
  describe('row B2’s slot conventions', () => {
    it('covers all 40 part-name sets, in the 32 / 4 / 4 split, out of three conventions', () => {
      ...
      /* Three, and every one of them reached by a fixture — which is again true
         now that row E3's `corridor` is withdrawn. It was the one convention in
         the table no upstream template could reach, and a convention nothing
         classifies is a layout with no piece to lay out. */
      expect(SLOT_CONVENTIONS).toHaveLength(3)
      expect(new Set(per.keys())).toEqual(new Set(SLOT_CONVENTIONS.map((convention) => convention.id)))
    })
  ...
  it('runs the convention gate at emit time, so a 21st fixture fails the import', () => {
    /* A part set no convention covers fails `npm run import:catalog` naming the
       entry, rather than reaching the browser with no layout. Demonstrated on an
       invented twenty-first fixture, because the 20 real ones all classify. */
    const invented: TemplateFixture = {
      source: 'blueprints.s2w.ceiling.yaml',
      name: 'S2W: Wall on Tile: Ceiling (Any, Modular)',
      type: 'blueprint',
      tags: ['object|tile'],
      parts: [
